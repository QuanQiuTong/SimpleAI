...
int main(int argc, char const *argv[])
{
    ...
    invoker_t main_screen = do_nothing; // Make sure it's properly initialized.
    ...
    button buttons[] =
        {button(
         ...
         button(M_LEFT, BODY_4, _T("Exit the Demo"), []()
                { closegraph(); system("pause"); exit(0); })};
...
#define get_stragegy(x, y, g) \
    ...
    for (int i = STEP_NUM, x = 0, y = 0, score = 0, cmd; i--;)
    {
        ...
        {
            ExMessage msg = getmessage(EM_MOUSE);                              // 绷不住了，我鼠标回报率1000Hz，导致鼠标消息不太对劲
            if (backward.check(msg.x, msg.y) && msg.message == WM_LBUTTONDOWN) // 所以必须按照这个顺序组织短路运算符，否则一单鼠标停止运动，画面就停机更新
                return 0;
        }
    }
